import { DisclosureSection, EntryModal, FormField, ModalActions, ModalSection } from "../components/EntryModal";
...
function RecurringEditModal({ editValues, setEditValues, userOptions, addUserOption, saving, onClose, onSave }: {
  ...
  const kind =
    (editValues.kind ?? "expense") === "incoming" ? "incoming" : "expense";
  const status =
    (editValues.status ?? "active") === "inactive" ? "inactive" : "active";

  return (
    <EntryModal
      title={`Edit recurring ${kind}`}
      subtitle="Update the schedule and details used for future entries."
      onClose={onClose}
      footer={
        <ModalActions
          onCancel={onClose}
    ...
    >
      ...
      >
        <button
          ...
          onClick={() =>
            setEditValues((values) => ({ ...values, kind: "expense" }))
          }
        >
          Expense
        </button>
        <button
          type="button"
          className={kind === "incoming" ? "active" : ""}
          onClick={() =>
            setEditValues((values) => ({ ...values, kind: "incoming" }))
          }
        >
          Incoming
        </button>
      ...
      <ModalSection
        title={kind === "expense" ? "Expense details" : "Incoming details"}
      >
        <div className="modal-form-grid">
          {kind === "expense" ? (
            <>
              ...
              <FormField label="Paid to">
                <input
                  ...
                  onChange={(event) =>
                    setEditValues((values) => ({
                      ...values,
                      recurringExpensePaidTo: event.target.value,
                    }))
                  }
                />
              </FormField>
              ...
              <FormField label="Paid by">
                <input
                  ...
                  onChange={(event) =>
                    setEditValues((values) => ({
                      ...values,
                      recurringIncomingPaidBy: event.target.value,
                    }))
                  }
                />
              </FormField>
      ...
      <DisclosureSection
        title="Notes"
        summary={editValues.notes ? "Added" : "Optional"}
      >
